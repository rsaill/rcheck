module Poly = struct
  ...
  let rec close : btype -> btype = function
    | ( T_INTEGER | T_BOOL | T_STRING | T_Set _ ) as ty -> ty
    | T_Power ty -> T_Power (close ty)
    | T_Product (ty1,ty2) -> T_Product (close ty1,close ty2)
...
module Mono = struct
  ...
  let rec close : Poly.btype -> btype = function
    | Poly.T_INTEGER -> T_INTEGER
    | Poly.T_BOOL -> T_BOOL
    | Poly.T_STRING -> T_STRING
    | Poly.T_Set s -> T_Set s
    | Poly.T_Power ty -> T_Power (close ty)
    | Poly.T_Product (ty1,ty2) -> T_Product (close ty1,close ty2)
    | Poly.T_Meta m -> T_Set ("#"^string_of_int m)
end
